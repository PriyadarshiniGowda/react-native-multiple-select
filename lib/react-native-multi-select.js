...
export default class MultiSelect extends Component {
  static propTypes = {
    ...
    hideDropdownOnScreenTouch: PropTypes.bool,
  ...
  static defaultProps = {
    ...
    onClearSelector: () => {},
    onToggleList: () => {},
    hideDropdownOnScreenTouch: false,
    ...
    return actualSelectedItems.map(singleSelectedItem => {
      ...
      return (
        ...
        >
          ...
          <TouchableOpacity
            onPress={() => {
              this._removeItem(item,selectedItems);
            }}
          >
            <Icon
              name="close-circle"
  ...
  _removeItem = (item,selectedItems) => {
    ...
    onSelectedItemsChange(newItems,"remove");
  };

  _removeAllItems = () => {
    const { onSelectedItemsChange } = this.props;
    // broadcast new selected items state to parent component
    onSelectedItemsChange([],"remove");
  };
  ...
  _addItem = () => {
    ...
    if (newItemName) {
      ...
      onAddItem(newItems);
      onSelectedItemsChange(newSelectedItems,"add");
      this._clearSearchTerm();
    }
  };

  _toggleItem = (item,selectedItems) => {
    const {
      single,
      uniqueKey,
      onSelectedItemsChange
    ...
    if (single) {
      ...
      onSelectedItemsChange([item[uniqueKey]],"toggle");
    } else {
      ...
      // broadcast new selected items state to parent component
      onSelectedItemsChange(newItems,"toggle");
    }
  };
  ...
  _getRow = item => {
    const { selectedItemIconColor, displayKey, styleRowList,selectedItems } = this.props;
    return (
      <TouchableOpacity
        disabled={item.disabled}
        onPress={() => this._toggleItem(item,selectedItems)}
        style={[
      ...
      itemList = (
        <FlatList
          ...
          ItemSeparatorComponent={() => {
            return <View style={{backgroundColor: '#707070',
            opacity: 0.1,
            height: 1,
            marginHorizontal:16
          }} />;
  ...
  componentDidUpdate(prevProps, prevState) {
    console.log(this.props.hideDropdownOnScreenTouch, "hidedropdown")
    if(prevProps.hideDropdownOnScreenTouch!== this.props.hideDropdownOnScreenTouch){
      this.setState({selector:false})
    }
    if(prevProps.addRTL!== this.props.addRTL){
      this.setState({addRTL:!this.state.addRTL})
    }
  }
                          ...
                          ? [
                              {
                                ...
                                color:
                                selectedItems.length === 0 ? textColor: '#000000',
